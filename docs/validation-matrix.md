...
| Match/mismatch arousal (Fig. 7) | Horizontal bottom-up plus horizontal top-down match versus vertical bottom-up plus horizontal top-down mismatch. | Match recruits more relay/TRN activity; mismatch disinhibits nonspecific thalamus and therefore produces a higher rate. | `structural` and `qualitative`; the paper publishes no numeric nonspecific-thalamus rates. |
