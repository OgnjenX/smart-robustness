"""Record source-resolved drive for every TRN cell in the fixed Figure 7 pair."""
...
def _condition_summary(result):
    ...
    return {
        ...
        "trn_event_count_by_index": [
            [index, counts.get(index, 0)] for index in range(81)
        ],
...
def main() -> None:
    ...
    default_indices = figure7_module.FIGURE7_RELAY_DIAGNOSTIC_INDICES
    if default_indices != (22, 31, 38, 39, 40, 41, 42, 49, 58):
        raise ValueError("unexpected default Figure 7 diagnostic index set")
    figure7_module.FIGURE7_RELAY_DIAGNOSTIC_INDICES = tuple(range(81))
        ...
        yaml.safe_dump(
            {
                ...
                "status": "completed-read-only-full-sheet-trn-localization",
                "classification": registration["classification"],
                "registration": args.registration,
                "runtime_fingerprint": conventions.fingerprint,
                "diagnostic_indices": list(range(81)),
                "persistent_projection_weight_scales": persistent_scales,
