...
def main() -> None:
    ...
    for gain in profile["dimension"]["grid"]:
        ...
        counts = {index: result.relay_spike_indices.count(index) for index in expected}
        gates = {
            "one_selected_category_event_during_lead": len(source_events) == 1,
            "no_off_source_category_events_during_lead": all(
                index == source_index for index in result.cue_lead_category_spike_indices
            ),
    ...
    survivors = [outcome["common_gain"] for outcome in outcomes if outcome["pass"]]
    artifact = {
        "schema_version": 1,
        ...
        "outcomes": outcomes,
        "match_survivor_gains": survivors,
        "selected_gain": min(survivors) if survivors else None,
        "assessment": {
