"""Run the fixed mismatch for the persistent projection-008 scale-0.875 model."""
...
def main() -> None:
    ...
    counts = Counter(int(index) for index in mismatch.trn_spike_indices)
    diagnostic = (22, 31, 38, 39, 40, 41, 42, 49, 58)
    diagnostic_total = sum(counts[index] for index in diagnostic)
    mismatch_summary = {
        "relay_event_count": len(mismatch.relay_spike_times_ms),
        "relay_active_indices": sorted(set(mismatch.relay_spike_indices)),
        "relay_spike_times_ms": list(mismatch.relay_spike_times_ms),
        "category_event_count": len(mismatch.category_spike_times_ms),
        "trn_event_count": len(mismatch.trn_spike_times_ms),
        "trn_diagnostic_index_event_count": diagnostic_total,
        "trn_remaining_index_event_count": len(mismatch.trn_spike_times_ms)
        - diagnostic_total,
        "nonspecific_event_count": len(mismatch.nonspecific_spike_times_ms),
        "nonspecific_spike_times_ms": list(mismatch.nonspecific_spike_times_ms),
    }
    gates = registration["fixed_mismatch_gates"]
    mismatch_summary["mismatch_gates"] = {
        "relay_active_indices": mismatch_summary["relay_active_indices"]
        == gates["relay_active_indices"],
        "match_more_active_relay_cells": 5
        > len(mismatch_summary["relay_active_indices"]),
        "match_more_trn_events": int(registration["fixed_match_trn_events"])
        > mismatch_summary["trn_event_count"],
        "nonspecific_events": mismatch_summary["nonspecific_event_count"]
        == gates["nonspecific_events"],
    }
    print(
        yaml.safe_dump(
            {
                ...
                "status": "completed-persistent-fixed-mismatch",
                "classification": registration["classification"],
                "registration": args.registration,
                "runtime_fingerprint": conventions.fingerprint,
                "persistent_projection_weight_scales": persistent_scales,
                "figure6_gates": figure6_gates,
                "fixed_match_trn_events": registration["fixed_match_trn_events"],
