...
| Match/mismatch arousal (Fig. 7) | 100 ms; horizontal bottom-up plus horizontal top-down match versus vertical bottom-up plus horizontal top-down mismatch. | Match recruits more relay/TRN activity; nonspecific thalamus fires 4 times (40 Hz) in match and 7 times (70 Hz) in mismatch. | `exact-source` plotted labels/spike counts plus `structural` pathway mechanism. |
