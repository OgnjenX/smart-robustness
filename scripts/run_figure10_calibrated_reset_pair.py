...
def _condition_summary(result):
    ...
    return {
        ...
        "projection026_target_arrival_summaries": [
            asdict(item) for item in result.projection026_target_arrival_summaries
        ],
...
def main() -> None:
    ...
    common = {
        ...
        "record_projection026_arrival_target_indices": tuple(
            int(index)
            for index in registration.get(
                "record_projection026_arrival_target_indices", ()
            )
        ),
        "projection026_arrival_window_ms": tuple(
            float(value)
            for value in registration.get(
                "projection026_arrival_window_ms", (23.3, 23.6)
            )
        ),
