"""Screen cue-safe TRN densities at the fixed event offset on match."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--grid-profile",
        default="configs/calibration/trn_density_event_offset_cross_v1.yaml",
    )
    parser.add_argument(
        "--stage-1-artifact",
        default=(
            "docs/validation-results/"
            "figure7-trn-density-event-offset-cue-grid-177.yaml"
        ),
    )
    ...
    densities = tuple(
        float(value) for value in stage_1["stage_1_survivor_densities_mS_cm2"]
    )
    if not densities or not set(densities) <= registered:
    ...
    for density in densities:
        conventions = replace(
            ...
            trn_spike_event_voltage_offset_mV=float(
                fixed["trn_spike_event_voltage_offset_mV"]
            ),
        )
        ...
        outcomes.append(
            {
                "trn_dendritic_calcium_density_mS_cm2": density,
                "trn_spike_event_voltage_offset_mV": fixed[
        ...
        print(
            f"density={density:g} mS/cm2: relay={len(result.relay_spike_times_ms)} "
    ...
    artifact = {
        ...
        "stage_2a_survivor_densities_mS_cm2": [
