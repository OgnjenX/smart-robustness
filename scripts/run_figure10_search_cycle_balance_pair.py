"""Run the preregistered passive event-locked Figure 10 balance pair."""
...
def main() -> None:
    ...
    common = {
        ...
        "audit_window_ms": tuple(float(value) for value in registration["audit_window_ms"]),
        "audit_bin_width_ms": float(registration["audit_bin_width_ms"]),
        "brian": brian,
    }
    intact = run_figure10_search_cycle_balance_condition(
        reset_pathway_enabled=True, **common
    )
    control = run_figure10_search_cycle_balance_condition(
        ...
        yaml.safe_dump(
            {
                ...
                "status": "completed-figure10-search-cycle-balance-pair",
