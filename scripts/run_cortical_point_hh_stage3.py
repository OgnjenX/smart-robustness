"""Run registered Figure 7 and Figure 10 conserved point-HH comparisons."""
...
def _validate_resume(raw, *, args, baseline, assessment) -> None:
    if raw.get("status") != "running-cortical-point-hh-stage3":
        raise ValueError("resume input is not a running point-HH stage-3 result")
    expected = {
        "baseline_manifest": args.baseline,
        "baseline_manifest_fingerprint": baseline.manifest_fingerprint,
        "runtime_fingerprint": baseline.runtime_fingerprint,
    ...
    unknown = set(arms) - {"classic_control", "cortical_point_hh_conserved"}
    if unknown:
...
def main() -> None:
    ...
    parser.add_argument(
        "--figure6-result",
        default="results/cortical-point-hh-figure6-929.yaml",
    )
    parser.add_argument(
        "--figure6-assessment",
        default=(
            "docs/validation-results/"
            "mechanism-cortical-point-hh-figure6-assessment-929.yaml"
        ),
    )
    ...
    for arm_index, (arm_name, population_factory) in enumerate(factories.items()):
        if any(
            later_name in arms
            for later_name in tuple(factories)[arm_index + 1 :]
        ...
        for repetition in range(len(outcomes), exact_reruns):
            ...
            output_path.write_text(
                yaml.safe_dump(
                    _checkpoint(
                        status="running-cortical-point-hh-stage3",
                        args=args,
                        baseline=baseline,
                        assessment=assessment,
                        arms=arms,
    ...
    point_pass = bool(
        arms["cortical_point_hh_conserved"]["all_repetitions_pass"]
    )
    completed = _checkpoint(
        status="completed-cortical-point-hh-stage3",
        args=args,
        baseline=baseline,
        assessment=assessment,
        arms=arms,
    ...
    completed.update(
        {
            ...
            "point_hh_stage3_pass": point_pass,
            "stage4_authorized": bool(classic_pass and point_pass),
        }
    )
