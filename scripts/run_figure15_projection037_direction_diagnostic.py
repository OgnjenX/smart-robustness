"""Run the preregistered projection-037 Figure 15 direction diagnostic."""
...
PROJECTION037_ID = "modeldb112923.projection.037"


def _isi_summary(spike_times_ms: np.ndarray) -> dict[str, float | int | None]:
    intervals = np.diff(spike_times_ms)
    if intervals.size == 0:
        return {"count": 0, "median_ms": None, "q25_ms": None, "q75_ms": None}
    return {
        "count": int(intervals.size),
        "median_ms": float(np.median(intervals)),
        "q25_ms": float(np.quantile(intervals, 0.25)),
        "q75_ms": float(np.quantile(intervals, 0.75)),
    }


def _load_generation_one(registration: dict) -> tuple[dict, dict, dict, dict]:
    holdout = yaml.safe_load(Path(registration["original_holdout_registration"]).read_text())
    profile = yaml.safe_load(Path(holdout["profile"]).read_text())
    ...
    return holdout, profile, training_profile, base_profile
...
def main() -> None:
    ...
    if PROJECTION037_ID in scales:
        raise ValueError("projection 037 unexpectedly differs from source scale")
    ...
    try:
        ...
        for scale in diagnostic["projection037_scales"]:
            arm_scales = dict(scales)
            arm_scales[PROJECTION037_ID] = float(scale)
        ...
        yaml.safe_dump(
            {
                ...
                "status": "completed-projection037-direction-diagnostic",
