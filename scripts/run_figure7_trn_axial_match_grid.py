"""Screen cue-safe local TRN axial scales on simultaneous Figure 7 match."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--grid-profile",
        default="configs/calibration/trn_soma_proximal_axial_behavior_grid_v1.yaml",
    )
    parser.add_argument(
        "--stage-1-artifact",
        default="docs/validation-results/figure7-trn-axial-cue-grid-173.yaml",
    )
    ...
    scales = tuple(float(value) for value in stage_1["stage_1_survivor_scales"])
    if not scales or not set(scales) <= registered:
    ...
    for scale in scales:
        conventions = replace(
            ...
            trn_soma_proximal_axial_conductance_scale=scale,
        )
        ...
        sampled_soma_peak_mV = max(
            maximum for _, _, maximum in result.trn_soma_voltage_range_mV_by_index
        )
        sampled_proximal_peak_mV = max(
            maximum for _, _, maximum in result.trn_proximal_voltage_range_mV_by_index
        )
        outcomes.append(
            {
                "trn_soma_proximal_axial_conductance_scale": scale,
                ...
                "sampled_trn_soma_peak_mV": sampled_soma_peak_mV,
                "sampled_trn_proximal_peak_mV": sampled_proximal_peak_mV,
        ...
        checkpoint = {
            ...
            "completed_scales": [
                item["trn_soma_proximal_axial_conductance_scale"] for item in outcomes
            ],
            "outcomes": outcomes,
        }
        output.write_text(yaml.safe_dump(_plain(checkpoint), sort_keys=False))
        print(
            f"scale={scale:g}: relay={len(result.relay_spike_times_ms)} "
            f"active={sorted(active_relay_indices)} trn={len(result.trn_spike_times_ms)} "
            f"soma_peak={sampled_soma_peak_mV:.3f} stage_2a={stage_2a_pass}",
            flush=True,
        )
    ...
    artifact = {
        ...
        "stage_2a_survivor_scales": [
            item["trn_soma_proximal_axial_conductance_scale"] for item in survivors
        ],
        "outcomes": outcomes,
