"""Test receptor-level simultaneous SMART expectation at sensory onset."""
...
def main() -> None:
    ...
    expected = set(FIGURE7_TOP_DOWN_EXPECTATION_PROJECTION_IDS)
    gates["all_six_expectation_records_primed"] = all(
        result.top_down_receptor_prime_present
        and {projection_id for projection_id, _ in result.top_down_receptor_prime_edge_counts}
        == expected
        and all(count > 0 for _, count in result.top_down_receptor_prime_edge_counts)
