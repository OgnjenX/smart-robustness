"""Repeat the fixed near-pass and preserve source-resolved TRN diagnostics."""
...
def _ranges(values):
    return [
        [int(index), str(source), float(minimum), float(maximum)]
        for index, source, minimum, maximum in values
    ]


def _condition_summary(result):
    counts = Counter(int(index) for index in result.trn_spike_indices)
    event_bins = Counter(int(np.floor(time_ms)) for time_ms in result.trn_spike_times_ms)
    return {
        ...
        "category_spike_times_ms": list(result.category_spike_times_ms),
        "trn_event_count": len(result.trn_spike_times_ms),
        "trn_spike_indices": list(result.trn_spike_indices),
        "trn_spike_times_ms": list(result.trn_spike_times_ms),
        "trn_event_count_by_index": [[index, counts.get(index, 0)] for index in range(81)],
        "trn_nonzero_one_ms_bins": [
            [bin_index, event_bins[bin_index]] for bin_index in sorted(event_bins)
        ],
        "trn_layer6ii_ampa_peak_by_index": _pairs(
            result.trn_layer6ii_ampa_peak_by_index
        ),
        "trn_layer6ii_nmda_peak_by_index": _pairs(
            result.trn_layer6ii_nmda_peak_by_index
        ),
        "trn_relay_ampa_peak_by_index": _pairs(result.trn_relay_ampa_peak_by_index),
        "trn_layer6ii_ampa_integral_ms_by_index": _pairs(
            result.trn_layer6ii_ampa_integral_ms_by_index
        ),
        "trn_layer6ii_nmda_integral_ms_by_index": _pairs(
            result.trn_layer6ii_nmda_integral_ms_by_index
        ),
        "trn_relay_ampa_integral_ms_by_index": _pairs(
            result.trn_relay_ampa_integral_ms_by_index
        ),
        "trn_driven_current_range_pA_by_index_and_source": _ranges(
            result.trn_driven_current_range_pA_by_index_and_source
        ),
        "trn_detector_threshold_upcrossings_by_index": [
            [int(index), int(value)]
            for index, value in result.trn_detector_threshold_upcrossings_by_index
        ],
        "trn_detector_release_transitions_by_index": [
            [int(index), int(value)]
            for index, value in result.trn_detector_release_transitions_by_index
        ],
        ...
        yaml.safe_dump(
            {
                ...
                "status": "completed-read-only-trn-drive-decomposition",
