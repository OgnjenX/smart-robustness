"""Test the literal Methods-global calcium density on the current Figure 6."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--profile",
        default="configs/calibration/figure6_methods_global_calcium_v1.yaml",
    )
    ...
    relay_counts = {index: relay_indices.count(index) for index in range(81)}
    relay_counts = {index: count for index, count in relay_counts.items() if count}
    recruitment = assess_figure6_cortical_recruitment(result)
    timing = assess_figure6_top_down_timing(result)
    gates = {
        "relay_active_indices": set(relay_indices) == {38, 39, 40, 41, 42},
    ...
    print(
        ...
        f"active={sorted(set(relay_indices))} "
        f"layer4={result.population_spikes['layer4_excitatory_v1']} "
        f"layer23={result.population_spikes['layer23_excitatory_v1']} "
        f"layer5={result.population_spikes['layer5_excitatory_v1']} "
        f"pass={passed}",
        flush=True,
    )
