"""Screen the predeclared behavior-calibration density grid for cue-only TRN output."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--grid-profile",
        default="configs/calibration/trn_dendritic_density_behavior_grid_v1.yaml",
    )
    ...
    fixed = grid_profile["fixed_source_choices"]
    densities = tuple(float(value) for value in grid_profile["dimension"]["grid"])
    bounds = tuple(float(value) for value in grid_profile["dimension"]["bounds"])
    if not densities or any(not bounds[0] <= value <= bounds[1] for value in densities):
        raise ValueError("density grid must be nonempty and remain within declared bounds")
    ...
    for density in densities:
        conventions = replace(
            ...
            trn_dendritic_calcium_density_mS_cm2=density,
        )
        ...
        checkpoint = {
            ...
            "completed_densities_mS_cm2": [
                item["trn_dendritic_calcium_density_mS_cm2"] for item in outcomes
            ],
            "outcomes": outcomes,
        }
        output.write_text(yaml.safe_dump(_plain(checkpoint), sort_keys=False))
        print(
            f"{density:g} mS/cm2: cue_trn={len(result.cue_lead_trn_spike_times_ms)} "
    ...
    artifact = {
        ...
        "stage_1_survivor_densities_mS_cm2": [
            item["trn_dendritic_calcium_density_mS_cm2"] for item in survivors
        ],
        "outcomes": outcomes,
        "next_gate": (
            "Evaluate only stage-1 survivors under paired simultaneous-onset "
            "match/mismatch conditions."
        ),
    }
