...
Artifacts 823--843 complete the permitted source-bounded direction tests for
the five relevant local layer-4 projections. None supplies a coherent monotonic
frequency direction: projection 037 is null, projections 030/036/028 mainly
control rate with non-monotonic peaks, and projection 029 has modest
non-monotonic effects. No local value or new generation is selected.

Artifacts 844--854 move from parameter probes to observable provenance. Every
pair among the five stimulated layer-4 cells has the same 53.026513-Hz peak,
so the unrecovered pair identity is not the cause within the source-compatible
row. A bit-exact event replay then shows matching 53-Hz relay, TRN, and layer-4
population peaks and a 5-ms relay-to-layer-4 lag. The local inhibitory
population emits only one startup volley. The sustained clock is therefore
relay-locked in this reconstruction rather than generated by a spiking local
E-I oscillator, without claiming causal direction from phase alone.

Artifacts 855--858 apply the final registered direction discriminator to the
three already-calibrated TRN-to-relay GABA transfer paths. Stronger joint
transfer monotonically slows relay and TRN peaks from 60 to 50 to 40 Hz, but
layer-4 population and fixed-pair frequencies do not follow coherently. The
registered criterion for a global generation-two grid fails, so the joint
factor, a denser grid, and a full-duration endpoint confirmation are closed.

This exhausts the bounded local and upstream one-parameter routes justified by
the current source and event evidence. Do not continue fitting Figure 15. The
next admissible action is to decide whether the reproducible candidate should
be frozen as a calibrated behavioral reconstruction—with Figures 6, 7, 10,
14, and 16 passing and source-identifiable Figure-15 gamma reproduced, but the
44-versus-53-Hz graphical discrepancy retained—or whether all downstream
