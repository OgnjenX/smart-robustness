"""Screen the registered local TRN axial grid for cue-only output."""
...
def main() -> None:
    ...
    scales = tuple(float(value) for value in grid_profile["dimension"]["grid"])
    if not scales or any(not np.isfinite(value) or value <= 0 for value in scales):
        raise ValueError("axial scale grid must be nonempty, finite, and positive")
    ...
    for scale in scales:
        ...
        print(
            f"scale={scale:g}: cue_trn={len(result.cue_lead_trn_spike_times_ms)} "
