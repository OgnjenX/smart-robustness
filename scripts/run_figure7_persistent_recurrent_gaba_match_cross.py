"""Screen a bounded persistent projection-008/011 recurrent-GABA match cross."""
...
def _match_summary(result, profile):
    ...
    central_total = sum(counts[index] for index in central)
    summary = {
        ...
        "trn_peripheral_event_count": len(result.trn_spike_times_ms) - central_total,
...
def main() -> None:
    ...
    somatic = registration["persistent_somatic_projection_scale"]
    if somatic["projection_id"] in base_scales:
        raise ValueError("somatic recurrent projection overlaps an existing scale")
    protocol = profile["protocol"]
    comparator = profile["comparator"]
    outcomes = []
    for proximal_scale in registration["persistent_proximal_projection_scale_grid"]:
        persistent_scales = {
            **base_scales,
            somatic["projection_id"]: float(somatic["scale"]),
            registration["proximal_projection_id"]: float(proximal_scale),
        }
        ...
        outcome = {
            "proximal_projection_scale": float(proximal_scale),
        ...
        yaml.safe_dump(
            {
                ...
                "status": "completed-persistent-recurrent-gaba-match-cross",
