"""Evaluate top-down-current survivors on the 300-ms Figure 7 pair."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--stage-2a-artifact",
        default="docs/validation-results/figure7-event-blend-current-match-grid-183.yaml",
    )
    ...
    currents = tuple(float(value) for value in stage_2a["stage_2a_survivor_currents_pA"])
    ...
    for current in currents:
        ...
        for condition in (MatchCondition.MATCH, MatchCondition.MISMATCH):
            ...
            print(
                f"current={current:g} pA {condition.value}: "
        ...
        gates = {
            ...
            "nonspecific_mismatch_nonzero": bool(
                mismatch.nonspecific_spike_times_ms
            ),
        }
    ...
    artifact = {
        ...
        "protocol": {
            ...
            "equilibration_ms": 20.0,
            "top_down_cue_lead_ms": 0.0,
            "scoring_window": "whole_epoch",
        },
        "stage_2b_survivor_currents_pA": [
            item["top_down_current_pA"] for item in survivors
        ],
        "outcomes": outcomes,
        "next_gate": (
            "Evaluate Figure 10 and Figures 14--16 only for Stage-2b survivors."
        ),
    }
