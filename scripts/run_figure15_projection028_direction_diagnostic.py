"""Run the preregistered projection-028 Figure 15 direction diagnostic."""
...
PROJECTION028_ID = "modeldb112923.projection.028"
...
def main() -> None:
    ...
    if PROJECTION028_ID in scales:
        raise ValueError("projection 028 unexpectedly differs from source scale")
    ...
    try:
        ...
        for scale in diagnostic["projection028_scales"]:
            arm_scales = dict(scales)
            arm_scales[PROJECTION028_ID] = float(scale)
            ...
            inhibitory_times_ms = np.asarray(
                result.interneuron_spike_times_ms, dtype=float
            )
            ...
            outcomes.append(
                {
                    "projection028_scale": float(scale),
        ...
        yaml.safe_dump(
            {
                ...
                "status": "completed-projection028-direction-diagnostic",
