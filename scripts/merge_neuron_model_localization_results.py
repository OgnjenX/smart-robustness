...
def main() -> None:
    ...
    for input_name in args.inputs:
        ...
        for arm_name, arm in payload["arms"].items():
            ...
            target = arms.setdefault(arm_name, {"trials": []})
            target["trials"].extend(arm["trials"])  # type: ignore[union-attr]

    for arm_name, arm in arms.items():
        trials = arm["trials"]
        seeds = [int(trial["seed"]) for trial in trials]
        if len(seeds) != len(set(seeds)):
            raise ValueError(f"duplicate GIF seed in {arm_name}")
        ...
        arm["trials"] = sorted(trials, key=lambda trial: int(trial["seed"]))
