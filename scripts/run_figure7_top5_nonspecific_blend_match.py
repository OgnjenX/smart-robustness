"""Screen the predeclared nonspecific event transfer on top-five match."""
...
def _write(
    ...
    *,
    ...
    survivors = [
        item["nonspecific_event_blend_fraction"]
    ...
    artifact = {
        ...
        "outcomes": outcomes,
        "match_survivor_blend_fractions": survivors,
        "selected_nonspecific_event_blend_fraction": selected,
        "assessment": {
...
def main() -> None:
    ...
    parser.add_argument(
        "--profile",
        default="configs/calibration/figure7_top5_nonspecific_blend_match_v1.yaml",
    )
    ...
    training = run_figure6_learning(
        conventions=baseline_conventions,
    ...
    expected_indices = {int(index) for index in gate["relay_active_indices"]}
    ...
    for fraction in profile["dimension"]["grid"]:
        ...
        gates = {
            "relay_active_indices": set(result.relay_spike_indices)
            == expected_indices,
            "relay_events": len(result.relay_spike_times_ms)
            == int(gate["relay_events"]),
            "trn_events": len(result.trn_spike_times_ms) == int(gate["trn_events"]),
        ...
        outcomes.append(
            {
                "nonspecific_event_blend_fraction": float(fraction),
        ...
        print(
            f"blend={float(fraction):g} relay={len(result.relay_spike_times_ms)} "
